...
def MTL_ours(x, y, r=3, T1=1, T2=0.05, R=5, r_bar=5, eta=0.05, max_iter=200, C1=1, C2=0.5, delta=0.05, adaptive=False):
    T = x.shape[0]
    n = x.shape[1]
    p = x.shape[2]
...
def MTL(x, y, r, eta=0.05, delta=0.05, max_iter=200):
    T = x.shape[0]  # T个任务
    n = x.shape[1]  # n个样本
    p = x.shape[2]  # p个属性
    A_hat = np.zeros((p, r))
...
def our_task(h):
    ...
    # MTL ours
    beta_hat_ours = MTL_ours(x=x, y=y, r=3, max_iter=200, eta=0.05)

    # MTL ours: adaptive
    beta_hat_ours_ad = MTL_ours(x=x, y=y, r=3, r_bar=sqrt(T), eta=0.05, max_iter=200, adaptive=True)

    # MTL the same representation
    beta_hat = MTL(x=x, y=y, r=3, eta=0.05, max_iter=200)
...
def our_task_outlier(h):
    ...
    # MTL ours: adaptive
    beta_hat_ours_ad = MTL_ours(x=x, y=y, r=3, r_bar=sqrt(T), max_iter=200, eta=0.05, adaptive=True)
    ...
    #result[1] = np.mean(all_distance(beta_hat_single_task, beta)[0:(T - 1)])
    #result[2] = np.mean(all_distance(beta_hat, beta)[0:(T - 1)])
    #result[3] = np.mean(all_distance(beta_hat_ours, beta)[0:(T - 1)])
    #result[4] = np.mean(all_distance(beta_hat_ours_ad, beta)[0:(T - 1)])
...
with open("C:/Users/samoh/PycharmProjects/MTLa/venv/demo.py" + str(0) + "_no_outlier_test.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerows(mse_no_outlier)

with open("C:/Users/samoh/PycharmProjects/MTLa/venv/demo.py" + str(0) + "_outlier_test.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
